    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 text-gray-800">
      <div ref={modalRef} className="bg-white p-6 rounded shadow-md w-full max-w-xl">
        ...
        <div className='flex flex-row'>
          ...
          <div className='flex flex-row w-3/4 pl-10'>
            ...
            <div className='flex flex-col items-start w-1/2'>
              ...
              <div className='text-base font-semibold'>{user?.birthday ? formatDate(user.birthday.toString()) : ' - - - '}</div>
              <div className='text-sm mt-4'>年齢</div>
              <div className='text-base font-semibold'>{age}</div>
              <div className='text-sm mt-4'>性別</div>
              <div className='text-base font-semibold'>{user?.gender === 'male' ? '男' : '女'}</div>
            </div>
          </div>
        </div>
        ...
        <div className='flex flex-row-reverse'>
          <button type='button' className='rounded-md px-4 py-1 bg-gray-400 hover:bg-gray-500 text-white font-bold text-base duration-300' onClick={onClose}>キャンセル</button>
          <button type='button' className='rounded-md px-10 py-1 mx-4 bg-blue-500 hover:bg-blue-600 text-white font-bold text-base duration-300' onClick={() => setIsSaveConfirmModal(true)}>保存</button>
        </div>
      </div>
