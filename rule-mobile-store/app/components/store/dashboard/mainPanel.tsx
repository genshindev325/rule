...
const MainPanel: React.FC<MainPanelProps> = ({
  ...
  return (
    <div className="py-4 sm:py-6 bg-gray-100 text-gray-800 space-y-4">
      <div className="w-full flex flex-row space-x-4">
        ...
        <div className="text-sm text-gray-600">前月の売上</div>
          <div className="text-base font-semibold">{formatNumber(lastMonthSales)} 円</div>
        </div>
        <div className="flex-1 border-gray-300 border-solid border p-4 rounded-md text-center bg-white">
        <div className="text-sm text-gray-600">今月の売上</div>
          <div className="text-base font-semibold">{formatNumber(thisMonthSales)} 円</div>
        </div>
      </div>
      <div className="w-full flex flex-row space-x-4">
        <div className="flex-1 border-gray-300 border-solid border p-4 rounded-md text-center bg-white">
        <div className="text-sm text-gray-600">予定イベント数</div>
          <div className="text-base font-semibold">{formatNumber(scheduledEvents)} 件</div>
        </div>
        <div className="flex-1 border-gray-300 border-solid border p-4 rounded-md text-center bg-white">
        <div className="text-sm text-gray-600">定員未達</div>
          <div className="text-base font-semibold">{formatNumber(unreachedCases)} 件</div>
        </div>
      </div>
      <div className="w-full flex flex-row space-x-4">
        <div className="flex-1 border-gray-300 border-solid border p-4 rounded-md text-center bg-white">
        <div className="text-sm text-gray-600">レビュー数</div>
          <div className="text-base font-semibold">{formatNumber(reviews)} 件</div>
        </div>
        <div className="flex-1 border-gray-300 border-solid border p-4 rounded-md text-center bg-white">
        <div className="text-sm text-gray-600">レビュー返信率</div>
          <div className="text-base font-semibold">{formatNumber(reviewResponseRate)}%</div>
        </div>
      </div>
    </div>
  );
